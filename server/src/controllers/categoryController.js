...
const createCategory = asyncHandler(async (req, res, next) => {
  try {
    ...
    const queryCreate =
      "insert into categories (name, parent_id, description, sort_order, image) values (?, ?, ?, ?, ?)";
...
const updateCategory = asyncHandler(async (req, res, next) => {
  try {
    ...
    const queryUpdate =
      "update categories set name = ?, parent_id = ?, description = ?, sort_order = ?, image = ?, updated_date = ? where id = ?";
    const values = [
      name,
      parent_id,
      description,
