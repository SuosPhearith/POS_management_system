...
const update = asyncHandler(async (req, res, next) => {
  try {
    ...
    const queryCheck =
      "select name from paymentTypes where name = ? AND id != ?";
    ...
    const updatePaymentType =
      "update paymentTypes set name = ?, description = ? where id = ?";
    const create = await executeQuery(updatePaymentType, [
      name,
      description,
      id,
    ]);
