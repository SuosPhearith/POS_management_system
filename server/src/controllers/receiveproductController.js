...
const getListReceiveproducts = asyncHandler(async (req, res, next) => {
  try {
    ...
    let querySearch = "";
    if (search !== "") {
      querySearch = `where product_id = '${search}' or supplier_id = '${search}'`;
    }
    const getListinvoices =
      "select * from receiveproducts " +
      querySearch +
      " ORDER BY id DESC LIMIT ?, ?"; // Move the LIMIT clause to this position
    const getList = await executeQuery(getListinvoices, [offset, pagination]);
    const getListTotal = "select count(id) as total_row from receiveproducts";
...
const updateReceiveproducts = asyncHandler(async (req, res, next) => {
  try {
    ...
    const queryUpdate =
      "update receiveproducts set product_id = ?, quantity = ?, purchase_price = ?, unit_price = ?, product_price = ?, special_price = ?, sub_total= ?, supplier_id = ?, user_id = ?, updated_date = ? where id = ?";
    const values = [
      product_id,
      quantity,
      purchase_price,
