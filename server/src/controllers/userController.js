...
const userRegister = asyncHandler(async (req, res, next) => {
  try {
    ...
    const queryCreate =
      "insert into users (username, password, fullname, contact, role_id, image) values (?,?,?,?,?,?)";
...
const userUpdate = asyncHandler(async (req, res, next) => {
  try {
    ...
    const queryUpdate =
      "update users set username = ?, fullname = ?, contact = ?, role_id = ?, image = ?, updated_date = ? where id = ?";
    const values = [
      username,
      fullname,
      contact,
